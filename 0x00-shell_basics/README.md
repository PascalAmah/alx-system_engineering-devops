...
TASKS
Note: When doing this project, Use text editor of your choice to create and access the file on insert mode. Ensure the first line always has # !/bin/bash, then the second line has the correct command/answer.
...
input: mkdir /tmp/my_first_directory (Create a directory inside the tmp directory)

7. Move the file betty from /tmp/ to /tmp/my_first_directory.
File: 7-movethatfile
input: mv /tmp/betty /tmp/my_first_directory/betty (Move file betty, which is located inside the tmp directory, to the my_first_directory, which is also located inside the tmp directory. This exercise required some dir traversing.)
...
12. Write a script that prints the type of the file named iamafile. The file iamafile will be in the /tmp directory when we run your script.
File: 12-file_type
...
File: 100-lets_move
input: mv [A-Z]* /tmp/u (Move all files that begin with a capital letter to /tmp/u)
...
File: 102-tree
input: mkdir -p welcome/to/school (Create directory 'welcome' in the current directory. Create a directory 'to' inside the directory 'welcome'. Create a directory 'school' inside the directory 'to'. The -p option creates any intermediate directories in the path argument.)
...
File: 103-commas
input:  ls -map* (List all files and directories of the current directory, separated by commas. Directory names should end with a /. The listing should be alph ordered, except for dot (.) or dot dot (..), which should be listed at the beginning.)
...
Creates a magic file called school.mgc that can be used with the command file to detect School data files. Holberton data files always contain "SCHOOL" at offset 0.
